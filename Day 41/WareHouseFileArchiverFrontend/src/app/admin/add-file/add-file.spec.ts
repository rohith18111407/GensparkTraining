...
describe('AddFileComponent', () => {
  ...
  it('should alert if no matching item found', () => {
    spyOn(window, 'alert');
    component.fileToUpload = new File(['x'], 'x.txt');
    component.allItems = [];
    component.fileForm.setValue({
      file: component.fileToUpload,
      itemName: 'Invalid',
    ...
    expect(window.alert).toHaveBeenCalledWith('No matching item found!');
  });

  it('should upload file on valid submit', fakeAsync(() => {
    spyOn(window, 'alert');
    spyOn(component.fileUploaded, 'emit');
    component.fileToUpload = new File(['x'], 'x.txt');
    const matchedItem = { id: '123', name: 'Item', categories: ['Cat'] };

    component.allItems = [matchedItem];
    component.fileForm.setValue({
      file: component.fileToUpload,
      itemName: 'Item',
      itemCategory: 'Cat',
    ...
    mockAdminService.uploadFile.and.returnValue(of({}));
  ...
  it('should handle file upload error', fakeAsync(() => {
    ...
    mockAdminService.uploadFile.and.returnValue(throwError(() => 'error'));
